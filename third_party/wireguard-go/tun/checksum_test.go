...
func TestPseudoHeaderChecksum(t *testing.T) {
	for _, addrLen := range []int{4, 16} {
		for length := 0; length <= 9001; length++ {
			...
			rng.Read(dstAddr)
			rng.Read(buf)
			phSum := pseudoHeaderChecksumNoFold(ipProtocolTCP, srcAddr, dstAddr, uint16(length))
			csum := checksum(buf, phSum)
			phSumRef := pseudoHeaderChecksumRefNoFold(ipProtocolTCP, srcAddr, dstAddr, uint16(length))
