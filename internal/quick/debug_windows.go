...
func writeWindowsDiagnostics(ctx context.Context, logger *log.Logger, name, phase string) {
	...
	script := strings.Join([]string{
		...
		"Get-NetAdapter -Name " + debugPowerShellQuote(name) + " -ErrorAction Continue | Format-List Name,InterfaceDescription,ifIndex,Status,MacAddress,LinkSpeed,DriverInformation | Out-String -Width 240 | Write-Output",
		"Write-Output 'Interface IP/DNS:'",
		"Get-NetIPConfiguration -InterfaceAlias " + debugPowerShellQuote(name) + " -ErrorAction Continue | Format-List InterfaceAlias,InterfaceIndex,IPv4Address,IPv6Address,IPv4DefaultGateway,IPv6DefaultGateway,DNSServer | Out-String -Width 240 | Write-Output",
		"Get-DnsClientServerAddress -InterfaceAlias " + debugPowerShellQuote(name) + " -ErrorAction Continue | Format-Table InterfaceAlias,AddressFamily,ServerAddresses -AutoSize | Out-String -Width 240 | Write-Output",
