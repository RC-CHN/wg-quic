...
`v0.3.5` 在接口和 session 统计（包括保留的 final snapshot）中增加
`quic_datagram_rcv_queue_len`、`quic_datagram_rcv_queue_drops` 和
`quic_datagram_rcv_queue_high_water`。这些跨平台字段可以区分“QUIC 包已经到达并被
对端确认”与“其中的 DATAGRAM 随后因应用接收队列已满而丢弃”。现场采集器会导出
当前深度、累计丢包 delta 和高水位。队列满事件与 debug 记录会按 session 合并
限频；这里的事件描述 WireGuard bind 队列，而对应累计计数仍包含每一个被拒绝的
数据包。
...
`v0.3.5` 二进制已经在 OpenWrt 25.12.5 的 `armsr/armv8` 与 `x86/64` QEMU
...
从 `v0.3.2` 开始，Linux Release 压缩包也包含 OpenRC 实例脚本：
